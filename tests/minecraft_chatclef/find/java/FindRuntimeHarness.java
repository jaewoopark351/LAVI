...
public class FindRuntimeHarness {
    ...
    public static void main(String[] args) throws Exception {
        var m = reset(); entity(m, 20); var t = start("find 마을 주민 report"); code(t, "FOUND");
        check(m.baritone.process.starts == 0 && m.baritone.explore.starts == 0 && m.behaviour.depth == 0, "report never moves");
        check(projected(t, "@find 마을 주민 report", false, false).equals("completed"), "matching success projected");
        check(projected(t, "@find 좀비 report", false, false).equals("unknown"), "wrong command not projected");
        check(projected(t, "@find 마을 주민 report", true, false).equals("unknown"), "stopped root not projected");
        check(projected(t, "@find 마을 주민 report", false, true).equals("unknown"), "stop bound not projected");
        ...
        m = reset(); t = start("find 주민 report"); code(t, "NOT_FOUND");
        check(t.outcome().scanComplete(), "report not-found is a completed local observation");

        m = reset(); var e = entity(m, 20); t = start("find 주민");
        ...
        m = reset(); e = entity(m, 20); t = start("find 주민"); e.removed = true; tick(t);
        ...
        m = reset(); e = entity(m, 20); t = start("find 주민"); m.world.entities.remove(e);
        ...
        m = reset(); e = entity(m, 20); t = start("find 주민"); e.pos = new Vec3d(100, 64, 0); tick(t);
        ...
        m = reset(); entity(m, 20); t = start("find 주민"); t.interrupt(null); expire(t); tick(t);
        check(!t.isFinished(), "operation age does not expire an interrupted approach");
        expireApproach(t); tick(t); code(t, "APPROACH_TIMEOUT");
        m = reset(); entity(m, 20); t = start("find 주민"); m.world = new ClientWorld(); tick(t); code(t, "SCOPE_CHANGED");
        m = reset(); m.player.alive = false; t = start("find 주민"); code(t, "PLAYER_UNAVAILABLE");
        m = reset(); for (int i = 0; i < 4096; i++) m.world.entities.add(new Entity(zombie, i + 20, 3, 64, 3));
        t = start("find 주민 report"); code(t, "SEARCH_LIMIT");
        ...
        t = start("find block 상자 report"); completeScan(t); code(t, "FOUND");
        ...
        t = start("find block 상자 report"); completeScan(t); code(t, "NOT_FOUND"); check(t.outcome().scanned() == 0, "unloaded report blocks ignored");
        m = reset(); m.world.floor = false; m.world.blocks.put(new BlockPos(10,64,0), new BlockState(chest,false));
        t = start("find block 상자"); code(t, "NO_APPROACH");
        m = reset(); var drop = new ItemEntity(itemType,10,new ItemStack(diamond),20,64,.5); m.world.entities.add(drop);
        t = start("find item 다이아몬드"); check(m.behaviour.walking.test(new BlockPos(20,64,0)), "stand off dropped item");
        ...
        t = start("find 새로운 몹 report"); code(t, "FOUND"); check(t.outcome().registryId().equals("my_mod:new_mob"), "mod registry without whitelist");
        m = reset(); Registries.ENTITY_TYPE.add("other:villager",new EntityType("entity.other.villager"));
        Language.INSTANCE.names.put("entity.other.villager","주민"); t = start("find 주민 report"); code(t, "AMBIGUOUS_TARGET");
        ...
        entity(m,10); t = start("find 새 주민 report"); code(t, "FOUND"); check(low.closed && high.closed, "resource readers closed");
        ...
        entity(m,10); t = start("find entity minecraft:villager report"); code(t, "FOUND"); check(t.outcome().languageWarnings() == 1, "broken translation fallback warning");
        m = reset(); entity(m,2); ChatClefDiagnostics.fail = true; t = start("find 주민"); code(t, "ARRIVED"); ChatClefDiagnostics.fail = false;
        m = reset(); var command = new FindCommand(); int[] callbacks = {0,0};
        command.run(m,"find 주민",()->callbacks[0]++); command.run(m,"find 좀비",()->callbacks[1]++);
        m.callbacks.get(0).run(); m.callbacks.get(1).run(); check(callbacks[0] == 1 && callbacks[1] == 1, "native callback isolation");
        boolean rejected = false; try {command.run(m,"find 주민 # injection",()->{});} catch(adris.altoclef.commandsystem.CommandException bad) {rejected=true;}
