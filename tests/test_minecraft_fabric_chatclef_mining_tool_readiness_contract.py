...
class MinecraftFabricChatClefMiningToolReadinessContractTests(unittest.TestCase):
    ...
    def test_saved_iron_pickaxe_policy_is_not_weakened(self):
        ...
        self.assertIn("if (!decisionMade && stack.getDamage()+30 > stack.getMaxDamage())", text)
        self.assertIn("shouldSave = !MiningRequirement.getMinimumRequirementForBlock(block).equals(MiningRequirement.IRON);", text)
        self.assertIn('decisionReason = shouldSave ? "LOW_DURABILITY_BLOCK_NOT_IRON_REQUIRED" : "LOW_DURABILITY_IRON_REQUIRED";', text)
        self.assertIn("return shouldSave;", text)
