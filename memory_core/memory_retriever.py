...
class MemoryRetriever:#20260622_kpopmodder: 현재 질문과 관련된 과거 사건을 검색한다.
    ...
    def retrieve(
        ...
        if (
            ...
            and not self.accuracy_first_raw_search
        ):
        ...
        if self.accuracy_first_raw_search:
        ...
        if recent_results:
            ...
            if self._should_expand_raw_search(query):
    ...
    def _retrieve_long_raw_results(
        ...
        long_events = self._get_raw_events_safely(
            limit=None,
            event_types=self.raw_search_event_types,
    ...
    def _retrieve_from_derived(
        ...
        for row in rows or []:
            if not self._is_eligible_derived_screen_row(row):
                continue
    ...
    def _is_eligible_derived_screen_row(self, row):#20260627_kpopmodder: Derived fallback requires repeated ScreenVision evidence by default.
        if str(row.get("kind", "")) != "screen_observation":
            return False#20260626_kpopmodder: Avoid turning repeated AI replies into authoritative memory.
