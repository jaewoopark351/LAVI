...
한국어 음성 대화, 로컬 LLM, GPT-SoVITS, VTube Studio 연동을 중심으로 개조 중인 Live AI Vtuber Interface 프로젝트입니다.
...
> LAVI는 입력 소스, STT/관찰, LLM, Safety Filter, TTS, VTube Studio 출력을 플러그인 기반 구조로 연결하는 Live AI Vtuber Interface 프로젝트입니다.
...
로컬 경로만 `config\chess_config.json`에 적습니다.
...
`config\chess_config.json`은 기본 Chess 설정 파일입니다.
파일이 없으면 예시 파일을 복사해서 실제 LC0 경로와 weight 경로를 적습니다.
...
## 12.3. StarCraft II / Ares-sc2 / ProBots 설정 메모
...
`Ares-sc2`는 Python 기반 StarCraft II 봇을 개발하기 위한 프레임워크입니다. 새 봇 개발은 보통 `ares-sc2-bot-template`에서 시작하고, 공식 문서와 예제 봇을 참고하는 것을 권장합니다.

기존 ProBots/Changeling 흐름에는 ProBots vs Human App을 받습니다:
...
`plugins\StarCraft2\runtime\jre\bin\java.exe`가 있으면 runtime에 Java 실행 파일이 이미 포함된 상태이므로, 보통 별도 시스템 Java 설치는 필요하지 않습니다.
...
아래의 `C:\Vtuber_Souorce_Code\LAVI`는 프로젝트 경로 예시입니다.
...
브라우저가 자동으로 열립니다.
수동으로 접속할 때의 기본 주소는 아래와 같습니다.
해당 포트가 이미 사용 중이면 47861, 47862처럼 다음 포트로 자동 이동합니다.
...
VTube Studio와 LAVI를 연동하려면 API 플러그인 접근을 허용해야 합니다.
