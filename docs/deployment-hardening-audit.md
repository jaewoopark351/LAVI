...
| VTube Studio UI 생성 시 바로 websocket 시작 | 두 경로를 구분해야 한다. `init() -> authenticate() -> connection.start()`는 startup init 경로이고, UI 버튼은 `create_ui() -> on_authenticate_click() -> authenticate()` 경로다. | `plugins/VtubeStudio/VtubeStudio.py:53-82`, `plugins/VtubeStudio/VtubeStudio.py:161-172`, `plugins/VtubeStudio/vtube_studio_core/connection/vtube_studio_connection.py:148-180` |
...
| profile source 파일 | P0 현재 승인 범위는 `requirements/profiles/core.in`, `requirements/profiles/full.in` |
| accelerator source 파일 | `requirements/accelerators/cpu.in`, `requirements/accelerators/cu130.in` |
| lock 생성 도구 | P0 현재 lock은 committed pin lock이다. hash lock 생성 도구 선정은 후속 결정 사항이다. |
| lock 생성 명령 | FUTURE: hash lock으로 전환할 때만 별도 승인 후 정의한다. |
| cu130 lock 생성 명령 추가 옵션 | 현재 `windows-py314-full-cu130.txt`는 `--extra-index-url https://download.pytorch.org/whl/cu130`을 포함한다. |
| installer가 설치할 파일 | raw requirements가 아니라 `requirements/locks/windows-py314-<Profile>-<Accelerator>.txt` |
| installer install 명령 | `python -m pip install -r requirements\locks\windows-py314-<profile>-<accelerator>.txt` |
| lock 파일명 | P0 현재 승인 범위는 `requirements/locks/windows-py314-core-cpu.txt`, `requirements/locks/windows-py314-full-cu130.txt` |
...
| VTube Studio websocket | `VtubeStudio.init`, `authenticate`, `VTubeStudioConnection.start`, `websocket_thread` | `plugins/VtubeStudio/VtubeStudio.py:53-82`, `plugins/VtubeStudio/VtubeStudio.py:168-179`, `plugins/VtubeStudio/vtube_studio_core/connection/vtube_studio_connection.py:148-180` |
| VTube Studio UI button path | `create_ui`, `on_authenticate_click`, `authenticate` | `plugins/VtubeStudio/VtubeStudio.py:161-172` |
...
| --- | --- | --- | --- |
| P0-1 dependency source of truth와 committed lock | 새 파일 `requirements/profiles/core.in`, `requirements/profiles/full.in`; 새 파일 `requirements/accelerators/cpu.in`, `requirements/accelerators/cu130.in`; 새 폴더 `requirements/locks/`; `requirements/dev.txt`; `requirements.txt`; `requirements_full.txt`; `requirements_org.txt`; `requirements_cuda13_success.txt` | Core/CPU와 기존 Full/cu130 경로만 P0 승인 조합으로 고정한다. `requirements.txt`는 compatibility entrypoint로 격하한다. historical requirements는 snapshot으로 문서화한다. | installer가 raw requirements가 아니라 committed lock을 설치한다. unsupported matrix는 다른 lock으로 조용히 fallback하지 않고 actionable error를 낸다. Python 3.14과 기존 cu130 pin은 보존된다. |
...
아래 명령은 P0 구현 후 현재 Core/CPU 경로를 검증하는 기준 명령이다.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\install_windows.ps1 -Profile Core -Accelerator CPU -Dev
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\install_windows.ps1 -Profile Core -Accelerator CPU -Dev
.\venv\Scripts\python.exe .\scripts\preflight.py --profile Core --accelerator CPU --offline
.\venv\Scripts\python.exe -m pip check
.\venv\Scripts\python.exe -m ruff check app_core\app_composer.py core\config_manager.py core\profile_resolver.py core\paths.py plugin_system\loader.py plugin_system\selection.py scripts\preflight.py scripts\smoke_startup.py tests\test_app_composer.py tests\test_repository_contract.py tests\test_smoke_startup.py tests\test_plugin_system_imports.py
.\venv\Scripts\python.exe -m pytest tests\test_app_composer.py tests\test_repository_contract.py tests\test_smoke_startup.py tests\test_plugin_system_imports.py
...
.\venv\Scripts\python.exe .\scripts\smoke_startup.py --production-config-smoke --timeout-sec 30
```
...
- P0 현재 Core CI는 대규모 test tree 이동 대신 명시적 파일 list를 사용한다.
- Core CI는 optional profile test file을 수집하지 않는다.
- optional CI와 profile별 test tree 분리는 P1/P2 후속 작업이다.
- optional dependency missing을 단순 skip으로 숨기지 않는다. 후속 optional profile CI에서는 missing dependency가 실패다.

Core CI 명령:

```powershell
.\scripts\install_windows.ps1 -Profile Core -Accelerator CPU -Dev
.\scripts\install_windows.ps1 -Profile Core -Accelerator CPU -Dev
.\venv\Scripts\python.exe -m compileall -q <tracked source paths>
.\venv\Scripts\python.exe -m pip check
.\venv\Scripts\python.exe -m ruff check app_core\app_composer.py core\config_manager.py core\profile_resolver.py core\paths.py plugin_system\loader.py plugin_system\selection.py scripts\preflight.py scripts\smoke_startup.py tests\test_app_composer.py tests\test_repository_contract.py tests\test_smoke_startup.py tests\test_plugin_system_imports.py
.\venv\Scripts\python.exe -m pytest tests\test_app_composer.py tests\test_repository_contract.py tests\test_smoke_startup.py tests\test_plugin_system_imports.py
.\venv\Scripts\python.exe scripts\smoke_startup.py --profile Core --accelerator CPU --offline --modules-config config\modules.core.json --timeout-sec 30
...
FUTURE Optional CI 예:

```powershell
python -m pip install -r requirements\locks\windows-py314-voice-cpu.txt
...
```text
tracked audit updates are visible through normal git diff after P0 implementation.
```

참고: `docs/deployment-hardening-audit.md`는 tracked 감사 문서다. pack-local 문서로 교체하지 않고 현재 저장소 구현과 검증 결과를 기준으로 갱신한다.
...
| 항목 | 이유 |
| --- | --- |
| clean Windows install 실제 성공 여부 | 현재 로컬 venv 기준 Core CPU installer 2회 성공을 확인했다. 완전한 새 VM/fresh checkout 검증은 GitHub Actions 또는 별도 장비에서 확인한다. |
| dependency lock 생성 가능 여부 | 현재 P0는 committed pin lock을 사용한다. 별도 lock refresh 도구 선정과 hash lock 전환은 후속 결정 사항이다. |
| `run.bat` startup 실제 결과 | Gradio 전체 runtime launch는 P0 smoke 범위를 넘어 실행하지 않았다. |
| Core offline smoke 실제 결과 | `scripts/smoke_startup.py --profile Core --accelerator CPU --offline --modules-config config\modules.core.json --timeout-sec 30` 성공. |
| production configuration smoke 실제 결과 | `scripts/smoke_startup.py --production-config-smoke --timeout-sec 30` 성공. |
| 네트워크 차단 상태 동작 | Core offline smoke guard에서 network/download/model/process attempt 0을 확인했다. OS/방화벽 수준 네트워크 차단 환경은 별도 장비에서 확인한다. |
| GPU 없는 PC 결과 | 현재 장비 조건을 변경하지 않았다. |
| GitHub Actions 결과 | 로컬에서 workflow와 동일한 핵심 명령을 실행했다. 실제 `windows-latest` 결과는 push/PR 후 확인한다. |
| root `modules.json` 모든 key와 dependency profile 일치 | production configuration smoke와 repository contract test로 schema/tracked/hash/resource-not-started 계약을 확인했다. 전체 optional dependency profile matrix 검증은 P1/P2 후속 범위다. |
| pip resolver 결과 | Core CPU committed lock 설치와 `pip check` 성공을 확인했다. |
