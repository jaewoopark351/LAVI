...
//20260904_kpopmodder: Parse the English trust command with an exact fail-closed grammar.
public final class AutoDepositEnglishTrustCommandFormParser {
    public Optional<AutoDepositTrustCommandForm> parse(ArgParser parser) {
        ...
        if (units.length == 0) {
            return Optional.of(AutoDepositTrustCommandForm.ENGLISH_SINGLE);
        }
        if (units.length != 2 || !"16x16".equals(units[1])) {
            return Optional.empty();
        }
        if ("area".equals(units[0])) {
            return Optional.of(AutoDepositTrustCommandForm.ENGLISH_AREA);
        }
        if ("\uBC18\uACBD".equals(units[0])) {
            return Optional.of(AutoDepositTrustCommandForm.ENGLISH_RADIUS);
        }
        return Optional.empty();
    }
}
