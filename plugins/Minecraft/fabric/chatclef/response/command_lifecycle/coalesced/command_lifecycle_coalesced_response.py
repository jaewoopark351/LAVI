#20260907_kpopmodder: Carry one immediate START-and-terminal sentence to presentation sinks.
...
class CommandLifecycleCoalescedResponse:
    text: str
    event_id: str
    command_name: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("command coalesced response event_id is invalid")
        if (
            type(self.command_name) is not str
            or self._COMMAND_NAME.fullmatch(self.command_name) is None
        ):
            raise ValueError("command coalesced response command_name is invalid")
...
__all__ = ("CommandLifecycleCoalescedResponse",)
