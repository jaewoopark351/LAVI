...
class MinecraftGetAndEquipAction:
    ...
    def run(self, item: Any, count: Any = 1) -> Dict[str, object]:
        blocked = self.permission_guard.blocked_response("get_and_equip")
        if blocked is not None:
            return blocked

        request = self.validator.validate("get_and_equip", item, count)
        ...
        return self.verified_runner.run(
            ...
            item=request["item"],
            count=request["count"],
            submit=lambda: self.client_provider.client.get_and_equip(
                request["item"],
                request["count"],
            ),
        )
