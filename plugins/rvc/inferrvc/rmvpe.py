...
class E2E(nn.Module):
    ...
    def forward(self, mel):
        # log_print(mel.shape)#20260612_kpopmodder
        mel = mel.transpose(-1, -2).unsqueeze(1)
        x = self.cnn(self.unet(mel)).transpose(1, 2).flatten(-2)
        x = self.fc(x)
        # log_print(x.shape)#20260612_kpopmodder
        return x
...
class RMVPE:
    ...
    def infer_from_audio(self, saudio: torch.Tensor | np.ndarray, thred=0.03):
        ...
        #log_print(audio.shape)#20260612_kpopmodder
        #log_print(audio.unsqueeze(0).shape)#20260612_kpopmodder
        ...
        # torch.cuda.synchronize()
        #t2 = ttime()
        # log_print(234234,hidden.device.type)#20260612_kpopmodder
        ...
        # torch.cuda.synchronize()
        #t3 = ttime()
        # log_print("hmvpe:%s\t%s\t%s\t%s"%(t1-t0,t2-t1,t3-t2,t3-t0))#20260612_kpopmodder
        return f0
    ...
    def to_local_average_cents(self, salience, thred=0.05):
        ...
        # t4 = ttime()
        # log_print("decode:%s\t%s\t%s\t%s" % (t1 - t0, t2 - t1, t3 - t2, t4 - t3))#20260612_kpopmodder
        return devided
