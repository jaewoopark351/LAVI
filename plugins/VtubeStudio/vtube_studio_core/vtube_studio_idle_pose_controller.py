#20260628_kpopmodder: Idle pose controller keeps a configurable resting expression when AI is not speaking.
import threading

from core.logger import log_print


class VTubeStudioIdlePoseController:#20260628_kpopmodder
    def __init__(
        self,
        send_callback,
        connected_callback,
        authenticated_callback,
        eye_open=0.52,#20260707_kpopmodder
        face_angle_x=0.0,
        face_angle_y=0.0,
        face_angle_z=0.0,
        eye_ball_x=0.0,
        ...
        self.idle_callback = idle_callback or (lambda: True)
    ...
    def stop(self):
        ...
        try:
            ...
            log_print(f"[VtubeStudio] idle pose stop error ignored: {e}")

        try:
            self.reset()
        except Exception as e:
            log_print(f"[VtubeStudio] idle pose reset error ignored: {e}")

    def is_active(self):
        with self.lock:
            return self.active

    def idle_pose_loop(self):
        while not self.stop_event.is_set():
            try:
                self.apply_idle_pose_once()
            except Exception as e:
                log_print(f"[VtubeStudio] idle pose loop error ignored: {e}")

            if self.stop_event.wait(self.refresh_interval_sec):
                break

    def apply_idle_pose_once(self):
        if (
    ...
    def is_overridden(self):
        try:
            ...
            log_print(f"[VtubeStudio] idle pose override check error ignored: {e}")
            return False

    def is_idle_allowed(self):
        try:
            return bool(self.idle_callback())
        except Exception as e:
            log_print(f"[VtubeStudio] idle pose state check error ignored: {e}")
            return False
    ...
    def is_eye_open_overridden(self):
        try:
            ...
            log_print(f"[VtubeStudio] idle pose eye override check error ignored: {e}")
            return False
