...
class ProBotsLogWatcher:
    ...
    def _poll_state(self, state: WatchedLogState) -> None:
        ...
        try:
            ...
            if file_size == state.offset:
                return
            with _compat_open(state.path, "rb") as file:
...
def _compat_open(*args, **kwargs):
    #20260717_kpopmodder: Preserve legacy tests that patch probots_log_watcher.open.
    from . import probots_log_watcher as compatibility_module

    opener = getattr(compatibility_module, "open", open)
