...
def tag_cjke(text):
    ...
    # log_print(sentences)#20260612_kpopmodder
    prev_lang=None
    tagged_text = ""
    for s in sentences:
        #全为符号跳过
...
def latest_checkpoint_path(dir_path, regex="G_[0-9]*.pth"):
    ...
    x = f_list[-1]
    log_print(f"latest_checkpoint_path:{x}")#20260612_kpopmodder
    return x
...
def oldest_checkpoint_path(dir_path, regex="G_[0-9]*.pth", preserved=4):
    ...
    if len(f_list) > preserved:
        x = f_list[0]
        log_print(f"oldest_checkpoint_path:{x}")#20260612_kpopmodder
        return x
    return ""
